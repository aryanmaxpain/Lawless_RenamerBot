...
from pyrogram import Client as RenamerSh, filters
...
@RenamerSh.on_message(filters.command("help") & filters.private & filters.incoming)
...
@RenamerSh.on_message(filters.command("start") & filters.private & filters.incoming)
async def start(c, m, cb=False):
    owner = await c.get_users(Config.OWNER_ID)
    owner_username = owner.username if owner.username else 'Lawless_Updates'
    button = [[
...
@RenamerSh.on_message(filters.command("about") & filters.private & filters.incoming)
...
@RenamerSh.on_message(filters.command("mode") & filters.private & filters.incoming)
...
@RenamerSh.on_message(filters.command("reset") & filters.private & filters.incoming)
async def reset_user(c, m):
    if m.from_user.id in Config.AUTH_USERS:
        ...
        await m.reply_sticker(sticker="CAACAgIAAxkBAAEEj2liZ6GJ2rvm3gr4yXGu8sm2-x8jDAAC6hMAAqqPoUuJgRRhGQ8OJSQE", quote=True)
        await m.reply_text(text="You are not admin to use this command.")


################## login command ##################

@RenamerSh.on_message(filters.command('login') & filters.incoming & filters.private)
async def password(c, m):
    if Config.BOT_PASSWORD:
        ...
        if len(m.command) == 1:
            await m.reply_text('Send me the bot password in the following format `/login password`')
        else:
            ...
            if pwd == Config.BOT_PASSWORD:
                ...
                await m.reply_text(f'Logged in Sucessfully to the bot.\nEnjoy the bot now {FACE_SAVORING_FOOD}.', quote=True)
            else:
                await m.reply_sticker(sticker="CAACAgIAAxkBAAEEj2liZ6GJ2rvm3gr4yXGu8sm2-x8jDAAC6hMAAqqPoUuJgRRhGQ8OJSQE", quote=True)
